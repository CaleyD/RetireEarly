      ...
      <View style={{
        ...
        {withdrawalRate > annualReturn ?
          <View style={{flexDirection: 'row'}}>
            <Ionicon name="md-warning" style={{fontSize: 30, color: 'orange'}}
              allowFontScaling={false}/>
            <Text>Withdrawal rate is higher than annual returns.</Text>
          </View> : null
        }
        <Text>
          Safe Withdrawal Rate: {(withdrawalRate * 100.00).toFixed(2)}%
        </Text>
        <Slider
          minimumValue={.01} maximumValue={.08} step={.0025}
          value={withdrawalRate}
          onValueChange={value=>this.setState({withdrawalRate: value})}
          onSlidingComplete={setWithdrawalRate}/>

        <Text>
          Annual Return: {(annualReturn * 100.00).toFixed(2)}%
        </Text>
        <Slider
          minimumValue={.01} maximumValue={.12} step={.0025}
          value={annualReturn}
