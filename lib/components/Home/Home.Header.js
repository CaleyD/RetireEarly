...
import {View, NavigationExperimental, Text, TouchableOpacity} from 'react-native';
      ...
      renderTitleComponent={() => {
        return (
          <NavigationHeader.Title>
            ...
            {'\n'}
            <Text style={{fontSize: 14}}>{formatMoney(retirementPortfolio)}</Text>
          </NavigationHeader.Title>
        );
      }}
        ...
        () => (
          <View style={{flexDirection: 'row'}}>
            ...
            <TouchableOpacity onPress={toggleViewExpanded} style={{paddingHorizontal: 8}}>
              <FontAwesome name={isExpanded ? "compress" : "expand"} style={{fontSize: 24, color: '#FFF'}} allowFontScaling={false}/>
            </TouchableOpacity>
          </View>
        )
      }
      renderLeftComponent={
        () => (
          <TouchableOpacity onPress={openMenu}>
            <Ionicon name="md-menu" style={{fontSize: 30, color: '#FFF'}} allowFontScaling={false}/>
          </TouchableOpacity>
        )
      }
    />
