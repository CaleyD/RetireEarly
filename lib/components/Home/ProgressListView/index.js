...
import {
  movePeriod, setInitialPortfolio, addPeriod, deletePeriod
...
import { memoized as calculate } from '../../../calculator.js';
...
class Progress extends Component {
  constructor (props) {
    super(props);
    this.state = {};
    this.expandableGroup = new ExclusiveExpandableGroup(()=>this.forceUpdate());
  ...
  getDataSource () {
    ...
    annualBalances.forEach(({balance: portfolioValue}, year) => {
  ...
  render () {
    const { scenario, outlook, onAddPeriod, onDeletePeriod } = this.props;
    const { data, order, incomeIndices } = this._source || this.getDataSource();
    return (
      <SortableListView
        automaticallyAdjustContentInsets={false}
        style={{flex: 1, backgroundColor: '#999'}}
        sortRowStyle={{opacity: 0.8, transform: [{scale: 1.05}]}}
        contentContainerStyle={{paddingBottom: 2}}
        scrollEnabled={order.length > 2 && !this.state.scrollDisabled}
        data={data} order={order} stickyHeaderIndices={incomeIndices}
        onRowMoved={e => this.onRowMoved(e)}
        ref={r=>{this.listView = r;}}
        renderHeader={() =>
          <View>
            <Graph width={320} height={140} outlook={outlook}
              onPanning={(panning)=>{
                this.setState({scrollDisabled: panning});
                this.listView.scrollTo({y: 0});
              }}/>
            <InitialPortfolioValueRow
              initialPortfolio={scenario.initialPortfolio}/>
          </View>
        }
        renderRow={rowData => {
            ...
            return (
              <View>
                ...
                <TouchableOpacity onPress={()=>this.toggleAddRow(rowData.year)}>
                  <YearRow {...rowData}
                    isPostFI=
                      {Math.ceil(outlook.yearsToRetirement) <= rowData.year}
                    retirementPortfolio={outlook.retirementPortfolio}
                    viewExpanded={this.props.viewExpanded}/>
                </TouchableOpacity>
              </View>);
          } else if (rowData.type === 'new-period') {
            return (
              <AnimateInOut
                deleted={this.expandableGroup.isCollapsed(rowData.year)}
                onDeleteAnimationComplete={this.onAddRowCollapsed}>
                <IncomeExpenseRowPlaceHolder {...rowData}
                  onAddPeriod={onAddPeriod} onDismiss={this.onAddRowDismissed}/>
              </AnimateInOut>
            );
          }
          return (
            <IncomeExpenseRow {...rowData}
  ...
  onRowMoved (e) {
    ...
    // annoying - the moved data is already at the destination index
    const targetYear = data[order[e.to - 1]].year + 1;
    this.forceUpdate();
    setImmediate(()=>{
      this._source = null;
      onMovePeriod(period, targetYear);
    });
  }
}
Progress.propTypes = {
  onMovePeriod: PropTypes.func.isRequired,
  scenario: PropTypes.object.isRequired,
  outlook: PropTypes.object.isRequired,
  hideDeleteAndReorder: PropTypes.bool,
  onInitialPortfolioChange: PropTypes.func.isRequired,
...
export default connect(
  ({scenario, ui}) => ({
    scenario: scenario,
    outlook: calculate(scenario),
    viewExpanded: ui.expanded
  }),
  {
    onMovePeriod: movePeriod,
    onInitialPortfolioChange: setInitialPortfolio,
