...
function Header({ outlook, dispatch, menuPressed, viewExpanded }) {
  return (
    <View style={styles.pageHeader}>
      ...
      <TouchableOpacity onPress={() => {dispatch(toggleMarketAssumptions())}} style={{paddingHorizontal: 8}}>
        <Ionicon name="md-options" color="#FFF" style={{fontSize: 24}}/>
      </TouchableOpacity>
      <TouchableOpacity onPress={() => dispatch(toggleViewExpanded())} style={{paddingHorizontal: 8}}>
...
export default connect(
  state => ({viewExpanded: state.viewDetails.expanded})
)(Header);
