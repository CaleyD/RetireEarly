...
class QuickEditPanel extends Component {
  constructor(props) {
    super(props);
    const {scenario, yearIndex, propName, visible} = props;
    this.state = {
      scenario: JSON.parse(JSON.stringify(scenario)),
      initialValue: visible ? scenario.incomePeriods[yearIndex][propName] : 0,
      propName
    };
  ...
  render() {
    const { yearIndex, visible, closeEditPeriod } = this.props;
      ...
      <View>
        ...
        <View style={{flex: 1, flexDirection: 'row'}}>
          ...
          <TouchableOpacity onPress={()=>this.setState({propName: 'income', initialValue: period.income})}>
            <Text style={{color: propName==='income' ? 'red' : 'black'}}>Income: {period.income}</Text>
          </TouchableOpacity>

          <TouchableOpacity onPress={()=>this.setState({propName: 'expenses', initialValue: period.expenses})}>
            <Text style={{color: propName==='expenses' ? 'red' : 'black'}}>Expenses: {period.expenses}</Text>
          </TouchableOpacity>
        </View>
        <View>
        ...
        <Dial style={styles.dial}
          ______value={numericTranslator.toDial(value)}
...
QuickEditPanel.propTypes = {
  ...
  closeEditPeriod: PropTypes.func.isRequired
}
...
export default connect(
  ({scenario, ui: {editingPeriod}}) => ({
    scenario,
    visible: !!editingPeriod,
    yearIndex: editingPeriod && editingPeriod.yearIndex,
    propName: editingPeriod && editingPeriod.propName
  }),
  {
    editPeriod, deletePeriod, setAnnualReturn,
    setWithdrawalRate, setInitialPortfolio, closeEditPeriod
  }
)(QuickEditPanel);
