    ...
    return (
      <View style={{marginTop: 80}}>
        <View style={{position: 'absolute', top: 0, left: 0, right: 0, bottom: 0}}
          onLayout={({nativeEvent: layout})=>this.setState({chartWidth: layout.width, chartHeight: layout.height})}>
          <Chart width={this.state.chartWidth || 320} height={this.state.chartHeight || 500} outlook={outlook}/>
        </View>

        <View style={{flex: 1, flexDirection: 'row', marginTop: 100}}>
          <TouchableOpacity onPress={()=>this.setState({propName: 'income', initialValue: income})}>
            <Text style={{color: propName==='income' ? 'red' : 'black'}}>
              Income: {formatMoney(income)}
            </Text>
          </TouchableOpacity>

          <TouchableOpacity onPress={()=>this.setState({propName: 'expenses', initialValue: expenses})}>
        ...
        <View>
          ...
          <Text>Years to financial independence: {outlook.yearsToRetirement}</Text>
        </View>
