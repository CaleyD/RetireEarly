...
export function calculate (scenario, doNotMemoize) {
  ...
  // {annualProjections: [{endOfYear, netWorth, annualROI}],
  // yearsUntilFI,
  // portfolioAtFI}
  ...
  incomePeriods.forEach((period, year) => {
    ...
    runningYearsToRetirement +=
      Math.min(yearsToRetirement, yearsTillNextPeriod);

    const endIndex = Math.min(100,
      yearsTillNextPeriod === Infinity ?
        yearsToRetirement : yearsTillNextPeriod
    );
    for (let i = 0; i < endIndex; ++i) {
      // this is an estimate
      // currently assumes all savings added at end of the year in one lump some
      runningPortfolioValue +=
        income - expenses + runningPortfolioValue * annualReturn;
      annualBalances.push({ balance: runningPortfolioValue });
    }
  });
  ...
  if (annualBalances.length < 80) {
    ...
    for (let i = annualBalances.length; i < 80; ++i) {
      ...
      // currently assumes all savings added at end of the year in one lump some
      const balance =
        annualBalances[annualBalances.length - 1].balance +
        income -
        expenses +
        annualBalances[annualBalances.length - 1].balance * annualReturn;
...
function getYearsUntilRetirement (
  ...
  if (annualIncome !== 0 && initialPortfolio !== 0) {
    ...
    numerator = Math.log(
      (1 - savingsRate) * (1 / withdrawalRate) * (annualReturn / savingsRate) +
      1);
  }
