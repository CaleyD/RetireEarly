...
module.exports.calculate = function(scenario) {
  ...
  incomePeriods.forEach(({income, expenses}, year) => {
    ...
    if(yearsTillNextPeriod === Infinity) {
      for(let i = 0; i < yearsToRetirement && i < 100; ++i) {
  ...
  return { retirementPortfolio, yearsToRetirement: runningYearsToRetirement, annualBalances };
