...
const expandedState = Object.freeze({expanded: true});
const collapsedState = Object.freeze({expanded: false});
const viewDetails = (state = expandedState, action) => {
  if(action.type === 'toggleViewExpanded') {
    return state.expanded ? collapsedState : expandedState;
  }
  return state;
};

const reducer = combineReducers({ scenario, viewDetails });
