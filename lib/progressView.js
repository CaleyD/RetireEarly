...
class IncomeExpenseRow extends PureComponent {
  ...
  componentWillMount() {
    this._panResponder = PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        this.setState({top: gestureState.dy + this._lastLayoutTop, moving: true});
  //      this.setNativeProps({top: gestureState.dy + this._lastLayoutTop});
      ...
      //  this.setState({top: 0, moving: false});
        ...
        scenarioStore.moveIncomPeriodToYearIndex(this.props.rowData.period, 3);
  ...
  render() {
    ...
    if(rowData.status === 'hovered') { // empty, new, populated, hovered
    ...
    return (
      <View
        ...
        style={[
          ...
          {top: this.state.top},
          this.state.moved ? {opacity: 0, height: 0, width: 0} : {},
          this.state.moving ? {
              ...
              <SidebarInputTrigger value={rowData[item.field]}
                ...
                renderChildren={(isSelected)=>(
                  <View style={isSelected ?
                    ...
                    <Text>{formatMoney(rowData[item.field])}</Text>
                  </View>
                )}/>
            </View>
          ))
  ...
  renderSorter() {
    ...
    return <Icon name="reorder" size={24} color="#BBB" />;
  }
}
  ...
  getDataSource() {
    ...
    annualBalances.forEach((entry, index) => {
      listItems.push({
        ...
        status: 'empty', // empty | new | populated | hovered
        year: index + 1
      });
      listItems.push({ type: 'year', portfolioValue: entry, year: index + 1 });
    });

    var currentYearNumber = 0;
    scenario.incomePeriods.forEach((period) => {
      let item = listItems[2*currentYearNumber];
      item.status = 'populated';
      item.annualIncome = period.annualIncome;
      item.annualSpending = period.annualSpending;
      item.period = period;

      incomeIndices.push(2*currentYearNumber + 1);
  ...
  render() {
    ...
    return (
      <ListView
        ...
        renderHeader={()=><InitialPortfolioValueRow scenario={this.props.scenario}/>}
        renderRow={(rowData) =>
          rowData.type === 'year' ?
            <View>
                ...
                <TouchableHighlight onPress={()=>this.setState({selectedRow: rowData.year===this.state.selectedRow ? null : rowData.year})}>
                  <View style={{flexDirection: 'row', height: 30, alignItems: 'center'}}>
                    ...
                    <Text style={styles.progressRowAmount}>
                      {formatMoney(rowData.portfolioValue)}
                    </Text>
                  </View>
                </TouchableHighlight>
              </ProgressRow>
