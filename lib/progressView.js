...
let SortableListView = require('./reacct-native-sortable-listview');

class InitialPortfolioValueRow extends PureComponent {
  render() {
    let year = new Date().getYear() + 1900 + this.props.year;
    let {initialPortfolio, dispatch} = this.props;
    return (
...
class IncomeExpenseRowPlaceHolder extends AnimateInOutRow {
  render() {
    let {dispatch, year, onCancel} = this.props;
    return (
...
class IncomeExpenseRow extends AnimateInOutRow {
  ...
  delete() {
    ...
    this.animateOut(() => this.props.dispatch(removeIncomePeriod(this.props.period)));
  }
}
...
class Progress extends PureComponent {
  ...
  getDataSource() {
    let {scenario: {incomePeriods}, outlook: {annualBalances = []}} = this.props;
    let data = {};
    let order = [];
    ...
    this.__source = { data, order, incomeIndices };
    return this.__source;
  }
  render() {
    let { scenario, outlook, hideDeleteAndReorder, dispatch } = this.props;
    let { data, order, incomeIndices } = this._source || this.getDataSource();
    return (
      <SortableListView
