...
class YearRow extends PureComponent {
  render() {
    let { year, portfolioValue, retirementPortfolio } = this.props;
    return (
      <View style={styles.progressRow}>
        <Text style={styles.progressRowYear}>{new Date().getYear() + 1900 + year}</Text>
        <TouchableHighlight style={{flex: 1}} onPress={()=>this.setState({selectedRow: year===this.state.selectedRow ? null : year})}>
  ...
  render() {
    return (
      <TouchableHighlight onPress={()=>{
        store.insertIncomePeriod(this.props.rowData.year-1);
      }}>
...
class IncomeExpenseRow extends PureComponent {
              ...
              <SidebarInputTrigger value={period[field]}
                ...
                renderChildren={(selected)=>(
                  <View style={selected ?
                    ...
                    <Text>{title}</Text>
                    <Text>{formatMoney(period[field])}</Text>
                  </View>
                )}/>
            </View>
          ))
  ...
  renderDeleteIcon() {
    ...
    return (this.props.allowDelete ?
        <TouchableHighlight onPress={()=>store.removeIncomePeriod(this.props.period)}>
    ...
    if(this.props.allowDelete) {
      return (
        <TouchableHighlight underlayColor={'#eee'} {...this.props.sortHandlers}>
          <Ionicon name="ios-reorder" size={30} color="#999" style={{width: 44, height: 30}}/>
        </TouchableHighlight>
      );
    }
    return <Ionicon name="ios-reorder" size={30} color="#BBB" style={{width: 44, height: 30}}/>;
  }
}
