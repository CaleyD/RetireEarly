...
class IncomeExpenseRow extends AnimateInOutRow {
  ...
  renderIncomeAndExpenses() {
    ...
    return (
      <View style={{flex: 1, flexDirection: 'row' }}>
        ...
        {[{title: 'Income', field: 'annualIncome', max: 500000},
          {title: 'Expenses', field: 'annualSpending', max: 100000}
        ].map(({title, field, max})=>(
  ...
  delete() {
    ...
    this.animateOut(() => store.removeIncomePeriod(this.props.period));
  }
}
...
class Progress extends PureComponent {
  ...
  onRowMoved(e) {
    ...
    setImmediate(()=>{
      this._source = null;
      store.moveIncomPeriodToYearIndex(period, destination-1)
    });
    return
