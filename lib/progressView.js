...
class IncomeExpenseRow extends PureComponent {
  ...
  renderDeleteIcon() {
    ...
    return (this.props.allowDelete ?
        <TouchableHighlight onPress={()=>store.removeIncomePeriod(this.props.period)} style={styles.iconContainer}>
...
class Progress extends PureComponent {
  ...
  getDataSource() {
    ...
    if(this.state.selectedRow != null) {
      data['new-period'] = { type: 'new-period', year: this.state.selectedRow };
      order.splice(currentYearNumber, 0, 'new-period');
    }
      ...
      <SortableListView
        ...
        renderRow={rowData => {
          if(rowData.type === 'year') {
            return (
              <TouchableHighlight
                onPress={()=>this.setState({selectedRow: rowData.year===this.state.selectedRow ? null : rowData.year})}>
                <YearRow {...rowData} retirementPortfolio={outlook.retirementPortfolio} />
              </TouchableHighlight>);
          } else if(rowData.type === 'new-period') {
            return <IncomeExpenseRowPlaceHolder {...rowData} onCancel={()=>this.setState({selectedRow: null})}/>;
          }
