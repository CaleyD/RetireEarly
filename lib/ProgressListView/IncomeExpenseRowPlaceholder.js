...
export default class IncomeExpenseRowPlaceHolder extends AnimateInOutMixin(Component) {
  constructor(props) {
    ...
    this.onCancel = () => this.animateOut(() => this.props.onDismiss(this.props.year));
    this.onAdd = () => {
  ...
  render() {
    const { year, onDismiss } = this.props;
    return (
      <Animated.View style={this.state.style}>
        <View style={[styles.incomeExpenseRow, {paddingHorizontal: 20}]}>
          <TouchableOpacity style={{flex: 1, alignItems: 'stretch', justifyContent: 'center'}} onPress={this.onAdd}>
