...
export default class Progress extends Component {
  ...
  getDataSource() {
    ...
    annualBalances.forEach((entry, year) => {
      ...
      // year row
      data[year] = { type: 'year', portfolioValue: entry, year };
      order.push(year);
    });
  ...
  render() {
    ...
    return (
      <SortableListView
        ...
        style={{flex: 1}} sortRowStyle={{opacity: 0.8, transform: [{scale: 1.05}]}}
  ...
  onRowMoved(e) {
    ...
    const targetYear = data[order[destination-1]].year + 1; // annoying - the moved data is already at the destination index
    this.forceUpdate();
    setImmediate(()=>{
      this._source = null;
