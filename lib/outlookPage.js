    ...
    return (
      <View style={{flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
        ...
        <View style={{flexDirection: 'row', flex: .8, marginTop: 5, marginBottom: 5}}>
          ...
          <TouchableHighlight
            onPress={()=>{
              if(this.state.selectedField==='annualIncome') {
                return;
              }
              this.setState({selectedField: 'annualIncome'});
              sidebarInput.show(
                rowData.annualIncome,
                (num)=>scenarioStore.updateIncomePeriod(rowData.period, {annualIncome: num}),
                ()=>this.setState({selectedField: null})
              );
            }}
            style={{flexDirection: 'row', height: 44}}>
            <View style={{flexDirection: 'column', flex: 1, alignItems: 'center',
                backgroundColor: this.state.selectedField==='annualIncome' ? 'orange' : 'transparent'}}
              >
              <Text>Income</Text>
              <Text>{formatMoneyCompact(rowData.annualIncome)}</Text>
            </View>
          </TouchableHighlight>

          <TouchableHighlight
            onPress={()=>{
              if(this.state.selectedField==='annualSpending') {
                return;
              }
              this.setState({selectedField: 'annualSpending'});
              sidebarInput.show(
                rowData.annualSpending,
                (num)=>scenarioStore.updateIncomePeriod(rowData.period, {annualSpending: num}),
                ()=>this.setState({selectedField: null})
              );
            }}
            style={{flexDirection: 'row', height: 44}}>
            <View style={{flexDirection: 'column', flex: 1, alignItems: 'center',
                backgroundColor: this.state.selectedField==='annualSpending' ? 'orange' : 'transparent'}}
              >
              <Text>Expenses</Text>
              <Text>{formatMoneyCompact(rowData.annualSpending)}</Text>
            </View>
          </TouchableHighlight>
